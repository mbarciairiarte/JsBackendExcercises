...
async function agregarNuevoProducto() {
  try {
    const title = await solicitarInformacion("Ingrese el título del producto: ", (input) => input.trim().length > 0);
    const description = await solicitarInformacion("Ingrese la descripción del producto: ", (input) => input.trim().length > 0);
    const price = await solicitarInformacion("Ingrese el precio del producto: ", (input) => !isNaN(parseFloat(input)) && parseFloat(input) >= 0);
    const thumbnail = await askQuestion("Ingrese la URL de la imagen (thumbnail) del producto: ");
    const code = await solicitarInformacion("Ingrese el código del producto: ", (input) => input.trim().length > 0);
    const stock = await solicitarInformacion("Ingrese la cantidad en stock del producto: ", (input) => !isNaN(parseInt(input)) && parseInt(input) >= 0);
...
async function buscarProductoPorId() {
  try {
    const id = await solicitarInformacion("Ingrese el ID del producto que desea buscar: ", (input) => input.trim().length > 0);
...
async function modificarProducto() {
  try {
    const id = await solicitarInformacion("Ingrese el ID del producto que desea modificar: ", (input) => input.trim().length > 0);
    const product = productManager.getProductById(id);

    const newTitle = await solicitarInformacion(`Ingrese el nuevo título del producto (antes: ${product.title}): `, (input) => input.trim().length > 0);
    const newDescription = await solicitarInformacion(`Ingrese la nueva descripción del producto (antes: ${product.description}): `, (input) => input.trim().length > 0);
    const newPrice = await solicitarInformacion(`Ingrese el nuevo precio del producto (antes: ${product.price}): `, (input) => !isNaN(parseFloat(input)) && parseFloat(input) >= 0);
    const newThumbnail = await askQuestion(`Ingrese la nueva URL de la imagen (thumbnail) del producto (antes: ${product.thumbnail}): `);
    const newCode = await solicitarInformacion(`Ingrese el nuevo código del producto (antes: ${product.code}): `, (input) => input.trim().length > 0);
    const newStock = await solicitarInformacion(`Ingrese la nueva cantidad en stock del producto (antes: ${product.stock}): `, (input) => !isNaN(parseInt(input)) && parseInt(input) >= 0);
...
async function eliminarProducto() {
  try {
    const id = await solicitarInformacion("Ingrese el ID del producto que desea eliminar: ", (input) => input.trim().length > 0);
